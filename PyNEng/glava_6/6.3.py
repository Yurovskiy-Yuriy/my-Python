'''
В скрипте сделан генератор конфигурации для access-портов. Сделать аналогичный генера-
тор конфигурации для портов trunk.
В транках ситуация усложняется тем, что VLANов может быть много, и надо понимать, что с
ним делать. Поэтому в соответствии каждому порту стоит список и первый (нулевой) элемент
...
Для данных в словаре trunk_template вывод на стандартный поток вывода должен быть та-
ким:
...
for intf, vlan in access.items(): # перебор ключ и значение
    print("interface FastEthernet" + intf)
    for command in access_template:
        if command.endswith("access vlan"): # прверка на что заканчивается значение
            print(f" {command} {vlan}")
        else:
            print(f" {command}")
print()
for intf_t, vlan_t in trunk.items():    # перебор ключ и значение
    print()
    print("interface FastEthernet" + intf_t)
    for command_t in trunk_template:
        if command_t.endswith("allowed vlan"):                 
            if vlan_t[0] == 'del':
                print(f" {command_t} remove {vlan_t[1]}")
            elif vlan_t[0] == 'only':
                print(f" {command_t} {vlan_t[1]} {vlan_t[2]}")
            else:
                print(f" {command_t} {vlan_t[0]} {vlan_t[1]} {vlan_t[2]}")
        else:
            print(f" {command_t}")
