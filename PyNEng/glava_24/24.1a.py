...
if __name__ == "__main__":
   ...
   if "device_type" not in device_params:
    device_params['device_type'] = input('Введите тип устройства: ')

   if "host" not in device_params:
      device_params['host'] = input('Введите IP-адресс устройства: ')

   if "username" not in device_params:
      device_params['username'] = input('Введите логин: ')
      
   if "password" not in device_params:
      device_params['password'] = input('Введите пароль: ')
      
   if "secret" not in device_params:
      device_params['secret'] = input('Введите пароль enable-режима: ')
   
    
   pr = CiscoSSH(**device_params)
   print(pr.send_show_command(command))
